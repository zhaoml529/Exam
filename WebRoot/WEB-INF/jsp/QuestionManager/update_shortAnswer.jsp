...
<html>
  <head>
    ...
    <title>My JSP 'update_shortAnswer.jsp' starting page</title>
...
<body>
	...
	<form action="<c:url value='/question/updateShortAnswer'/>" method="post" onsubmit="return validateForm(this)">
		<table class="default" width="1100px">
				<tr class="title">
					<td colspan="2">查看试题详细信息</td>					
				</tr>
				<tr><td>简答题编号</td>
					<td><input type="text" name="question_id" readonly="readonly" value="${shortAnswer.question_id }"/></td>
				</tr>
				<tr>
				<td>简答题题目</td>
				<td><input type="text" name="question_title"
					value="${shortAnswer.question_title }" />
				</td>
				</tr>
				<tr><td>试题描述</td>
					<td>
						<textarea id="editor_id1" name="question_body" style="width:700px;height:300px;">
						${shortAnswer.question_body }	
						</textarea>
					</td>
				</tr>
				<tr><td>正确答案</td>
					<td>
						...
						${shortAnswer.question_answer }	
						</textarea>
					</td>
				</tr>
				<tr><td>试题难度</td>
					<td><input type="text" name="difficulty" value="${shortAnswer.difficulty }"/></td>
				</tr>
				<tr><td>所属科目</td><td>
					<select name="subject.subject_id" emptyInfo="请选择试题科目">
							<c:forEach items="${subjectList}" var="subjectList">
								<option value="${subjectList.subject_id}" ${subjectList.subject_id == shortAnswer.subject.subject_id?'selected':"" }>${subjectList.subject_name}</option>
							</c:forEach>
						</select>
								</td></tr>
				<tr><td>年级</td><td>
				<select name="grade.grade_id" emptyInfo="请选择试题年级">
							<c:forEach items="${gradeInfoList}" var="gradeInfoList">
								<option value="${gradeInfoList.grade_id}" ${gradeInfoList.grade_id == shortAnswer.grade.grade_id?'selected':"" }>${gradeInfoList.grade_name}</option>
							</c:forEach>
						</select>
								</td></tr>
				<tr><td>添加人</td>
					<td><input type="text" name="add_person" readonly="readonly" value="${shortAnswer.add_person }"/></td>
				</tr>
				<tr><td>添加时间</td>
					<td><input type="text" name="add_time" value="${shortAnswer.add_time }" onclick="calendar.show(this);"/></td>
				</tr>
				<tr><td>试题分值</td>
					<td><input type="text" name="question_score" value="${shortAnswer.question_score }"/></td>
				</tr>
				<tr><td>试题备注</td>
					<td>
						<textarea id="editor_id3" name="question_question_remark" style="width:700px;height:300px;">
						${shortAnswer.question_remark  }	
						</textarea>
					</td>
				</tr>

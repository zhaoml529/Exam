...
<html>
  <head>
    ...
    <title>My JSP 'list_role.jsp' starting page</title>
  ...
  <body>
				...
				<c:otherwise>
					<c:forEach items="${roleList }" var="roleList">
						<tr class="changeColor">
							...
							<td>
								<a href="<c:url value='/right/revokeRoleById/${roleList.role_id }'/>">分配权限</a>
								|
								<a href="<c:url value='/right/updateRoleById/${roleList.role_id }'/>">修改</a>
								|
								<a href="<c:url value='/right/deleteRole/${roleList.role_id }'/>" onclick="return confirm('确定删除此功能吗?');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:otherwise>
	...
	<table align="center" width="500">
    	...
    	<a href="<c:url value="/right/listRole/pageNum/1"/>">首页</a>
    	<c:if test="${pageBean.hasPreviousPage == true}"><a href="<c:url value="/right/listRole/pageNum/${pageBean.prePage}"/>">上一页</a></c:if>
    	<c:if test="${pageBean.hasNextPage == true}"><a href="<c:url value="/right/listRole/pageNum/${pageBean.nextPage}"/>">下一页</a></c:if>
    	<a href="<c:url value="/right/listRole/pageNum/${pageBean.totalPage}"/>">尾页</a>
    	转到
    	<select id="pageNum">
    	...
    	页 
     	<input type="button" width="30px" value="GO" onclick="jumpPage('<c:url value='/right/listRole/pageNum/'/>');">
    	</td>
    	</tr>
    </table>
  </body>
